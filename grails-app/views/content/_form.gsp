...
<div class="fieldcontain ${hasErrors(bean: contentInstance, field: 'unit', 'error')} required">
	<label for="unit">
		<g:message code="content.unit.label" default="Unit" />
		<span class="required-indicator">*</span>
	</label>
	<g:select id="unit" name="unit.id" from="${nl.jappieklooster.kook.quantification.Unit.list()}" optionKey="id" required="" value="${contentInstance?.unit?.id}" class="many-to-one"/>
</div>
